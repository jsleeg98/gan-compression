    ...
    def __init__(self, opt):
        ...
        if opt.isTrain:
            if opt.bi_level_train:
                wandb_log_name = f'{opt.log_name}_FDRU_{"Y" if opt.mac_front else "N"}{"Y" if opt.mac_downsample else "N"}{"Y" if opt.mac_resnet else "N"}{"Y" if opt.mac_upsample else "N"}_tr_{opt.target_ratio}_mac_{opt.alpha_mac}_nuc_{opt.alpha_nuc}_biStart_{opt.bi_level_start_epoch}_biInterval_{opt.bi_level_interval}_Rmax_{opt.R_max}'
            else:
                wandb_log_name = f'{opt.log_name}_FDRU_{"Y" if opt.mac_front else "N"}{"Y" if opt.mac_downsample else "N"}{"Y" if opt.mac_resnet else "N"}{"Y" if opt.mac_upsample else "N"}_tr_{opt.target_ratio}_mac_{opt.alpha_mac}_nuc_{opt.alpha_nuc}_Rmax_{opt.R_max}'
